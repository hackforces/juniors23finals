...
class Checker(BaseChecker):
    ...
    def check(self):
        ...
        try:
            res = self.mch.register(session, username, password)
        except Exception as e:
            self.cquit(Status.DOWN)

        token_register = session.cookies.get('token', '')
        self.assert_neq(token_register, '', f'no authentication token provided after registration')
        session.close()
        session = self.mch.session_with_requests()
        try:
            self.mch.login(session, username, password)
        except:
            self.cquit(Status.DOWN, "cannot login")
        ...
        logging.debug('проверяем, что сайт вообще открывается')
        # проверяем, что сайт вообще открывается
        res = session.get(self.mch.url)
        soup = BeautifulSoup(res.text, "html.parser")
        # проверяем, что комментарии создаются
        for post in soup.findAll('li', class_='list-group-item d-flex justify-content-between align-items-start')[0:1]:
            post_id = post.find('h1', 'fw-bold').text.split('. ')[0]
            session.post(self.mch.url, data={'post_id': post_id, 'comment': rnd_string(length=50)})
        ...
        for post in soup.findAll('li', class_='list-group-item d-flex justify-content-between align-items-start'):
            ...
            if post_id == cur_post_id:
                if new_title == cur_post_title:
                    self.cquit(Status.OK)
        self.cquit(Status.MUMBLE, f"post not found")
    ...
    def put(self, flag_id: str, flag: str, vuln: str):
        ...
        post_id = self.mch.does_post_exist_by_title(session, title)
        if not post_id: self.cquit(Status.MUMBLE, 'cannot post')
        flag_id = f'{username}_{password}_{post_id}_{title}'
        self.cquit(Status.OK, flag_id, flag_id)

    def get(self, flag_id: str, flag: str, vuln: str):
        session = self.mch.session_with_requests()
        username, password, post_id, title = flag_id.split('_')
        self.mch.login(session, username, password)
        temp_post_id, flag_by_title = self.mch.does_post_exist_by_title(session, title, need_body=True)
        temp_post_id, flag_by_id = self.mch.does_post_exist_by_id(session, post_id, need_body=True)
